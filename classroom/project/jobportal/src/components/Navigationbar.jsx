...
export default function Navigationbar() {
  ...
  const renderNavItems = () => {
    ...
    return navItems.map((item) => {
      return (
        <Nav.Link as={Link} to={item.path} className="me-5">
          {item.name}
        </Nav.Link>
      );
    });
  ...
  return (
    <Container fluid className="px-0">
      <Navbar expand="md" className="bg-body-tertiary px-0">
        <Navbar.Brand as={Link} to="/" className="me-5 ms-3">
          LOGO
        </Navbar.Brand>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          ...
          {renderNavItems()}
          <Nav.Link as={Col} lg="5" className="me-5">
        ...
        <Dropdown>
          <Dropdown.Toggle
            id="dropdown-basic"
            className="rounded-circle bg-body-tertiary border-0"
          >
            <i class="bi bi-person-circle px-4 fs-1 text-dark"></i>
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item as={Link} to="/profile">
              <i class="bi bi-person-fill-add fs-5"></i> Profile
            </Dropdown.Item>
